# ==================================================================================
# BASELINE_RESNET18.PY - Simple Baseline for Glacier Segmentation
#
# This script establishes a baseline using:
# - ResNet18 U-Net (lightweight, ~11M params)
# - Basic augmentation (flips, rotations)
# - Dice + BCE Loss
# - 5-fold Cross-Validation
#
# GOAL: Establish baseline MCC (expected: 0.55-0.62)
# TRAINING TIME: ~30 minutes on Kaggle 2xT4
# ==================================================================================
...
class ResNet18UNet(nn.Module):
    """U-Net with ResNet18 encoder (pretrained on ImageNet)"""
    ...
    def __init__(self, n_classes=1, n_channels=5):
        ...
        # Copy pretrained weights for RGB channels, initialize extra channels
        with torch.no_grad():
            # Average RGB weights for each of the 5 channels
        ...
        self.decoder3 = self._make_decoder_block(256 + 256, 128)  # +256 from skip connection
        self.decoder2 = self._make_decoder_block(128 + 128, 64)   # +128 from skip connection
        self.decoder1 = self._make_decoder_block(64 + 64, 64)     # +64 from skip connection
...
class GlacierDataset(Dataset):
    """Dataset for glacier segmentation"""
    ...
    def _load_label(self, tile_id):
        ...
        for name in possible_names:
            ...
            if os.path.exists(path):
                ...
                if label.ndim == 3:
                    label = label[:, :, 0]
                return (label > 127).astype(np.float32)  # Binary: 0 or 1
    ...
    def __getitem__(self, idx):
        ...
        if self.stats:
            for i in range(5):
                mean = self.stats['means'][i]
                std = self.stats['stds'][i]
                if std > 0:
...
def compute_global_stats(data_dir, tile_ids):
    """Compute mean and std for each band across all training data"""
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(all_tile_ids)):
        ...
        # Create datasets
        train_dataset = GlacierDataset(CONFIG['data_dir'], train_ids, stats, augment=True)
        val_dataset = GlacierDataset(CONFIG['data_dir'], val_ids, stats, augment=False)
        ...
        for epoch in range(CONFIG['epochs']):
            ...
            if val_mcc > best_mcc:
                best_mcc = val_mcc
                patience_counter = 0
                model_path = os.path.join(CONFIG['work_dir'], f'best_baseline_fold{fold+1}.pth')
    ...
    with open(os.path.join(CONFIG['work_dir'], 'baseline_results.json'), 'w') as f:
